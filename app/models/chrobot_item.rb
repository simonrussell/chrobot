class ChrobotItem < ActiveRecord::Base
  ...
  ALL_STATUSES = [PENDING, ALLOCATED, STARTED, FINISHED, FAILED]
  ...
  named_scope :pending, :conditions => { :status => PENDING }, :order => 'priority DESC, run_at'
  ...
  def self.counts_per_status
    ALL_STATUS_COUNTS_ZERO.merge(count(:all, :group => 'status').to_hash)
  end
